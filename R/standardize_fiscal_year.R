...
#'   \code{"Q1FY23"}. Quarter and year may be separated by a single space or dash.
...
#'   returned as \code{NA} with a warning.Only exact 2-digit or 4-digit years
#'   are accepted; 3-digit or 5-digit years are treated as unrecognized.
#' @export
#'
#' @examples
...
standardize_fiscal_year <- function(x, prefix = "FY") {
  ...
  for (i in seq_along(x)) {
    val <- trimws(gsub("\\s+", " ", x[i]))

    if (grepl("^FY(\\d{2}|\\d{4})$", val, ignore.case = TRUE)) {
      year <- sub("^FY(\\d+)$", "\\1", val, ignore.case = TRUE)
      year <- expand_year(year)
      result[i] <- paste0(prefix, year)

    } else if (grepl("^\\d{4}$", val)) {
      result[i] <- paste0(prefix, val)
    } else if (grepl("^Q[1-4][\\s-]\\d{4}$", val, perl = TRUE, ignore.case = TRUE)) {
      parts <- strsplit(val, "[\\s-]", perl = TRUE)[[1]]
      result[i] <- paste0(prefix, parts[2], "-", toupper(parts[1]))
    } else if (grepl("^\\d{4}[\\s-]Q[1-4]$", val, perl = TRUE, ignore.case = TRUE)) {
      parts <- strsplit(val, "[\\s-]", perl = TRUE)[[1]]
      result[i] <- paste0(prefix, parts[1], "-", toupper(parts[2]))
    } else if (grepl("^Q[1-4]FY(\\d{2}|\\d{4})$", val, ignore.case = TRUE)) {
      quarter <- toupper(substr(val, 1, 2))
      year <- sub("^Q[1-4]FY(\\d+)$", "\\1", val, ignore.case = TRUE)
      year <- expand_year(year)
      result[i] <- paste0(prefix, year, "-", quarter)
